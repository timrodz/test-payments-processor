...
vi.mock("@/lib/client/@tanstack/react-query.gen", () => ({
  ...
  readPaymentByIdApiV1PaymentsPaymentIdGetOptions: vi.fn(() => ({ queryKey: ["pay"] })),
  readRegistrationByIdApiV1RegistrationsRegistrationIdGetOptions: vi.fn(() => ({ queryKey: ["reg"] })),
}));

describe("ConfirmationPage", () => {
  const mockTrip = { id: "trip-1", title: "Science Museum", location: "London", date: "2024-05-20" };
  const mockReg = { id: "reg-1", student_name: "Johnny Doe", parent_email: "jane@example.com" };
  const mockPay = { id: "pay-1", amount: 25.0, transaction_id: "TXN123", registration_id: "reg-1" };

  beforeEach(() => {
    vi.clearAllMocks();
    (useParams as any).mockReturnValue({ trip_id: "trip-1", payment_id: "pay-1" });
    // Mock window.print
  ...
  it("renders confirmation details", () => {
    (useQuery as any).mockImplementation((options: any) => {
      ...
      if (options.queryKey[0] === "pay") return { isLoading: false, data: mockPay };
    ...
    expect(screen.getByText("Johnny Doe", { selector: "p span" })).toBeDefined();
    expect(screen.getByText("Science Museum", { selector: "span" })).toBeDefined();
